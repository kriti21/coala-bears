...
class AnalyzeGitCommitTest(unittest.TestCase):
    ...
    def test_get_head_commit_sha(self):
        ...
        test_sha = run_shell_command('git rev-parse HEAD')[0].strip('\n')
        get_parents = 'git log --pretty=%P -n 1 ' + test_sha
        parents = run_shell_command(get_parents)[0].split(' ')
        self.assertEqual(self.run_uut(),
                         [('Add testfile\n\n', test_sha, parents,
                           COMMIT_TYPE.simple_commit, ['testfile.txt'],
                           [], [])])

        run_shell_command('git commit -m "Invalid commit"')
        self.assertRaisesRegex(RuntimeError,
                               r'The directory is not a git repository.', None)
